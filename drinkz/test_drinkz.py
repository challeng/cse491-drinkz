...
def test_total_ml_oz():
    ...
    assert db.get_liquor_amount('Johnnie Walker', 'Black Label') == "31573 ml"
...
def test_total_ml():
    ...
    assert db.get_liquor_amount('Johnnie Walker', 'Black Label') == "3000 ml"
...
def test_get_liquor_inventory():
    ...
    for mfg, liquor, amount in db.get_liquor_inventory():
        x.append((mfg, liquor, amount))

    assert x == [('Johnnie Walker', 'Black Label', '1000 ml')], x
